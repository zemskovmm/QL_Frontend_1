...
export function dmap<TValue, TRes>(d: { [key: string]: TValue }, cb: (key: string, value: TValue) => TRes): TRes[] {
  const rv: TRes[] = [];
  for (const k in d) {
    if (d.hasOwnProperty(k)) rv.push(cb(k, d[k]));
  }
  return rv;
}

export function dictMap<TValue, TRes>(
  d: { [key: string]: TValue },
  cb: (key: string, value: TValue) => TRes
): { [key: string]: TRes } {
  const rv: { [key: string]: TRes } = {};
  for (const k in d) {
    if (d.hasOwnProperty(k)) rv[k] = cb(k, d[k]);
  }
  return rv;
}
  ...
  let rv = {
    onChange: (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (model[setter] != null) {
        model[setter](value);
      } else model[name as string] = value;
    },
  } as any;
  if (type == "checkbox") rv.checked = model[name as string];
  else rv.value = model[name as string];
