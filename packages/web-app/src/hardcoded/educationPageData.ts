...
const educationPageData: EducationPageDataDto = {
  breadcrumbs: {
    fr: [
      {
        name: "Principal",
  ...
  firstScreen: {
    ru: "МЫ НА СОБСТВЕННОМ ОПЫТЕ ЗНАЕМ, КАК СЛОЖНО ПОДОБРАТЬ И ПОСТУПИТЬ В ЖЕЛАЕМОЕ УЧЕБНОЕ ЗАВЕДЕНИЕ",

    cn: "如果您在选择或者注册法国高等院校时遇到了困难",
    fr:
    ...
    esp: "SABEMOS POR EXPERIENCIA LO DIFÍCIL QUE PUEDE SER ENTRAR EN LA UNIVERSIDAD QUE UNO QUIERE.",
    en: "WE KNOW FROM EXPERIENCE HOW HARD IT IS TO CHOOSE A UNIVERSITY AND GET THROUGH THE ADMISSION PROCESS. ",
  },
  links: [
    {
      fr: {
  ...
  linksTitle: {
    ...
    cn: "我们为您的法国留学提供定制服务，为您选择最合适您的好学校",
  },
  reasonsList: {
    fr: {
      title:
  ...
  sponsorTitle: {
    fr: "NOUS SOMMES PARTENAIRES DE GRANDS ETABLISSEMENTS DE FRANCE ET D’AILLEURS",
    en: "WE WORK WITH THE BEST UNIVERSITIES IN FRANCE AND EUROPE",
    esp: "COLABORAMOS CON LOS MEJORES CENTROS DE FRANCIA Y DE OTROS PAÍSES",
    ru: "СОТРУДНИЧАЕМ С ВЕДУЩИМИ ВУЗАМИ ФРАНЦИИ И ДРУГИХ СТРАН",
    cn: "我们在法国和全世界法语地区都有众多合作院校",
  },
  allReviews: {
    cn: {
