...
export function Catalog<T>(props: CatalogProps<T>) {
  ...
  const data = siteApi.useCatalogItems<T>(locale, props.apiElementName, 10, parsed.page, parsed.filters);
  ...
  const setFilter = async (identifier: string, id: number | number[], selected: boolean) => {
    ...
    let newItems =
      typeof id === "object" ? selectedItems.filter((x) => !id.includes(x)) : selectedItems.filter((x) => x != id);
