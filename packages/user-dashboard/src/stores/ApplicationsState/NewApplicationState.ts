...
import { ApplicationType, APPLICATION_DTO_DEFAULT } from "@project/components/src/interfaces/ApplicationDto";
...
const clearCreateApplication = action(newApplicationState, "clearCreateApplication", (store) => {
...
const sendCreateApplication = action(
  ...
  async (store): Promise<void> => {
    ...
    if (createApplicationReq) {
      const result = await applicationsApi.addApplications({
